...
app.post('/api/couriers', (req, res) => {
    ...
    const courier = service.createCourier(id, x, y);
    res.json(courier);
});
...
app.post('/api/orders', (req, res) => {
    ...
    const order = service.createOrder(id, pickupX, pickupY, dropX, dropY);
    res.json(order);
});

// POST /api/assign: Trigger assignment for all pending orders
// Or specific order. For simplicity let's just try to assign a specific order or all pending.
// Let's do specific order for control.
app.post('/api/assign/:orderId', (req, res) => {
    ...
    try {
        ...
        if (courier) {
            ...
            // Check if it was because no couriers were available
            const anyFree = service.couriers.some(c => !c.isBusy);
            if (!anyFree) {
                return res.json({ success: false, status: "No couriers available" });
            }
...
app.post('/api/stage1', (req, res) => {
    const { x, y } = req.body;
    const result = service.findNearestCourier(parseInt(x), parseInt(y));
